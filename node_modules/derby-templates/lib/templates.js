...
saddle.AttributesExpression.prototype.dependencies = function(context) {
  return getDependencies(this.expression, context);
};
...
View.prototype.appendTo = function(parent, context) {
  var viewContext = this._initComponent(context);
  var template = this.template || this.parse();
  this._bindAttributes(viewContext, context, node);
...
View.prototype._bindAttributes = function(viewContext, context, node){
  if (!this._isComponent(context)) return;
  console.log('_bindAttributes', viewContext)
  if (!context.attributes) return
  for (var key in context.attributes) {
    var attribute = context.attributes[key];
    var segments = (
      attribute instanceof ParentWrapper &&
      attribute.expression &&
      attribute.expression.pathSegments(context)
    );
    if (!segments && attribute instanceof ParentWrapper) {
      var comonentAttribute = new saddle.ComponentAttribute(viewContext.controller, key, attribute);
      comonentAttribute.getBound(context, node, key);
      console.log('create component attribute binding', key)
    }
  }
}
