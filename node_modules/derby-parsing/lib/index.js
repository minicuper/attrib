...
function parseAttributes(attributes) {
  ...
  for (var key in attributes) {
    ...
    var DynamicAttributeClass = key === 'attributes' ? templates.AttributesExpression : templates.DynamicAttribute;
      ...
      attributesMap[key] =
        ...
        (item instanceof templates.DynamicText) ?
          (item.expression instanceof expressions.LiteralExpression) ?
            ...
            new DynamicAttributeClass(item.expression, nsUri) :
           new DynamicAttributeClass(item, nsUri);
...
function createExpression(source, supportObject) {
  source = source.trim();
  ...
  try {
    expression = (path) ?
      createPathExpression(path, supportObject) :
